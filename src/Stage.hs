...
voidObjects (width, height) = leftAndRight ++ aboveAndBottom
  where
    leftAndRight = [ObjState x y D.Down (ObjKindObj OVoid) False 0 | x <- [-1, width], y <- [0 .. height -1]]
    aboveAndBottom = [ObjState x y D.Down (ObjKindObj OVoid) False 0 | x <- [0 .. width -1], y <- [-1, height]]
